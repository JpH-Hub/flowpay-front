...
export default function StatusTag({ status }) {
  ...
  if (status === TICKET_STATUS.QUEUED) {
    return <span className="inline-flex rounded bg-yellow-100 px-2 py-1 text-[11px] font-bold text-yellow-800">Na Fila</span>
  }
  if (status === TICKET_STATUS.IN_SERVICE) {
    return <span className="inline-flex rounded bg-blue-100 px-2 py-1 text-[11px] font-bold text-blue-800">Em Atendimento</span>
  }
  if (status === 'CLOSED') {
